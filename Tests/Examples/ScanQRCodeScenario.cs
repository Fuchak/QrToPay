...
public static class ScanQRCodeScenario
{
    public static ScenarioProps GetScenario(HttpClient httpClient, int rate, TimeSpan interval, TimeSpan during)
    {
        return Scenario.Create("scan_qr_code_scenario", async context =>
        {
            string qrCode = DataGenerator.GetRandomQRCode();

            HttpRequestMessage request = Http.CreateRequest("GET", $"{StaticFields.baseUrl}api/Scan/{qrCode}")
